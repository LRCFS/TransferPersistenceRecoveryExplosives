  ...
  world.country$RegionEdit <- gsub(" ", "_", world.country$region)
  
  # create two lists to apply to correction
  CountryOriginalName <- as.character(world.country.Narrow$region)
  CountryEditedName <- as.character(world.country.Narrow$regionEdit)
  ...
  for(d in 1:nrow(ScopusAffiliationsFig5)){
    for(w in 1:length(CityOriginalName)){
        ScopusAffiliationsFig5$Affiliations[d] <- mgsub(ScopusAffiliationsFig5$Affiliations[d],(CityOriginalName)[w],(CityEditedName)[w])
        }
    print(d)
  }
  
  ##replace country name containing space with equivalent
  ScopusAffiliationsFig5$Affiliations <- mgsub((ScopusAffiliationsFig5$Affiliations),(CountryOriginalName),(CountryEditedName))
  ...
  AffiliationsCities <- subset(AffiliationsNarrowExtCityDetails, (Inst.exp %in% world.cities$NameEdit))
