...
# =========================================================
# GC-MS PETN/RDX Quantification Script (15N-RDX IS for PETN)
# =========================================================
#
...
#       * RDX:  1/x weighted quadratic (with linear fallback)
...
#       * PETN uses 15N-RDX (m/z 122) as IS -- ratio = PETN PA / 15N-RDX IS PA
#       * RDX uses 15N-RDX (m/z 122) as IS -- ratio = RDX PA / 15N-RDX IS PA
#  - PETN drift correction on both PA and ratio (QC-based)
#
# This is a variant of MetadataLinearWeighted.R that uses 15N-RDX
# as the internal standard for PETN instead of NG. NG is unreliable
# in V9 method sequences (often not detected).
#
...
required_globals <- c(
  ...
  "petn_is_config",      # list(mz, rt) for PETN internal standard (NG) -- legacy, not used for ratio
  ...
  "apply_petn_drift_correction"  # TRUE/FALSE: enable QC-based PETN drift correction (PA and ratio)
)
...
# =========================================================
# PETN IS (NG) Peak Area QC Chart (legacy diagnostic)
# =========================================================
...
if (nrow(petn_is_plot_data) > 0) {
  p_petn_is <- ggplot(petn_is_plot_data, aes(x = Row, y = petn_is_pa, colour = Group)) +
    ...
    theme_bw() +
    labs(
      title = "PETN IS (NG) Peak Area Across Sequence (Legacy Diagnostic)",
      x = "Injection Number",
...
# =========================================================
# Both PETN and RDX use 15N-RDX (rdx_is_pa) as their IS:
#   PETN ratio = PETN PA / 15N-RDX IS PA (m/z 122)
#   RDX ratio  = RDX PA / 15N-RDX IS PA (m/z 122)
...
#   "ratio" -- calibrate on (analyte PA / 15N-RDX IS PA)

# Map analyte to IS label for plot y-axis
analyte_is_label <- list(
  PETN = "PA / 15N-RDX IS Ratio",
  RDX  = "PA / 15N-RDX IS Ratio"
)
...
# Storage for PETN ratio calibration models, needed for drift-corrected
# ratio re-quantification after the calibration loop.  Keyed by cal set index.
stored_petn_ratio_models <- list()
...
for (analyte_name in names(analytes)) {
  ...
  for (i in seq_along(CalSets)) {
    ...
    for (method_name in names(cal_methods)) {
      ...
      # Store PETN ratio models for drift-corrected ratio re-quantification
      if (analyte_name == "PETN" && method_name == "ratio") {
        stored_petn_ratio_models[[i]] <- list(
          lin_model   = lin_model,
          quad_model  = quad_model,
          cal_y_range = cal_y_range,
          rows        = rows,
...
if (nrow(cal_stats_df) > 0) {
  ...
  # PETN IS %RSD per calibration set (legacy -- NG)
  ...
  # PETN IS %RSD overall (all injections) -- legacy
...
if (apply_petn_drift_correction && has_calibration &&
  ...
  if (nrow(qc_candidates) > 0) {
    ...
    if (is.finite(ref_pa) && ref_pa > 0) {
      ...
      n_levels_qc <- length(unique(qc_candidates$CalLevel))
      if (n_levels_qc > 1) {
        message("  PA drift correction: using QC level ", max_qc_cal_level,
                " ng (", nrow(qc_drift), " points; ",
                n_levels_qc - 1, " lower QC level(s) excluded)")
      } else {
        message("  PA drift correction: using ", nrow(qc_drift),
                " QC points at ", max_qc_cal_level, " ng")
      }
      ...
      if (n_qc >= 4) {
        ...
        if (is.na(r2) || r2 < 0.5) {
          message("  PA drift correction: quadratic R2 = ", round(r2, 3),
        ...
        drift_model  <- lm(PA_CorrFactor ~ Row + I(Row^2), data = qc_drift)
        drift_method <- "Quadratic"
        message("  PA drift correction: only 3 QC points -- quadratic fit is exact (0 residual df).")

      } else if (n_qc == 2) {
        drift_model  <- lm(PA_CorrFactor ~ Row, data = qc_drift)
        drift_method <- "Linear"
        message("  PA drift correction: only 2 QC points -- using linear interpolation.")

      } else {
        message("  PA drift correction: fewer than 2 usable QC points -- skipping correction.")
      }
      ...
      if (!is.null(drift_model)) {
        ...
        if (length(cal_rows) > 0) {
          ...
          if (is.finite(anchor_cf) && anchor_cf > 0) {
            ...
            message("  PA drift correction: anchored to Cal set 1 (mean Row ",
        ...
        if (any(predicted_cf < 0, na.rm = TRUE)) {
          warning("  PA drift correction: some predicted correction factors were negative -- clamping to 1.0.")
          predicted_cf[predicted_cf < 0] <- 1.0
        }
        if (any(predicted_cf > 3.0, na.rm = TRUE)) {
          warning("  PA drift correction: some correction factors exceed 3.0 -- severe drift detected.")
        }
        ...
        message("  PETN PA drift correction applied:")
      ...
      message("  PA drift correction: reference Cal PA is invalid -- skipping correction.")
      Combined$petn_drift_model <- "None"
    }
  } else {
    message("  PA drift correction: no usable QC rows -- skipping correction.")
    Combined$petn_drift_model <- "None"
  }

} else if (!apply_petn_drift_correction) {
  Combined$petn_drift_model <- "Disabled"
  message("  PETN PA drift correction: disabled (apply_petn_drift_correction = FALSE).")
}

# =========================================================
# PETN Ratio-Level Drift Correction (QC-based)
# =========================================================
# Corrects PETN/15N-RDX ratio for residual progressive drift
# that the IS cannot fully compensate. Uses the same QC-based
# approach as the PA drift correction but operates on the
# ratio values.
# ---------------------------------------------------------

# Initialise ratio drift-correction columns to uncorrected defaults.
Combined$petn_ratio_drift_correction_factor <- 1.0
Combined$petn_ratio_drift_model             <- NA_character_
Combined$petn_ratio_dc                      <- Combined$petn_ratio
Combined$petn_concentration_ratio_dc        <- Combined$petn_concentration_ratio
Combined$petn_quant_method_ratio_dc         <- Combined$petn_quant_method_ratio
Combined$petn_range_flag_ratio_dc           <- Combined$petn_range_flag_ratio

if (apply_petn_drift_correction && has_calibration &&
    "petn_ratio" %in% names(Combined) &&
    length(stored_petn_ratio_models) > 0) {

  # --- Step 1: Identify the highest QC level and reference ratio ---
  qc_candidates_ratio <- Combined %>%
    filter(Type == "QC", !is.na(petn_ratio), !is.na(CalLevel))

  if (nrow(qc_candidates_ratio) > 0) {
    max_qc_cal_level_ratio <- max(qc_candidates_ratio$CalLevel, na.rm = TRUE)

    # Reference ratio: mean PETN ratio of Cal standards at the QC level
    # from the first calibration set (earliest injections, minimal drift).
    ref_ratio <- Combined %>%
      filter(Type == "Cal",
             CalLevel == max_qc_cal_level_ratio,
             !is.na(petn_ratio),
             CalibrationSet == 1) %>%
      summarise(ref_ratio = mean(petn_ratio, na.rm = TRUE)) %>%
      pull(ref_ratio)

    if (is.finite(ref_ratio) && ref_ratio > 0) {

      # --- Step 2: Compute ratio correction factors at each QC position ---
      qc_drift_ratio <- Combined %>%
        filter(Type == "QC",
               CalLevel == max_qc_cal_level_ratio,
               !is.na(petn_ratio)) %>%
        mutate(Ratio_CorrFactor = ref_ratio / petn_ratio) %>%
        filter(is.finite(Ratio_CorrFactor), Ratio_CorrFactor > 0)

      n_levels_qc_ratio <- length(unique(qc_candidates_ratio$CalLevel))
      if (n_levels_qc_ratio > 1) {
        message("  Ratio drift correction: using QC level ", max_qc_cal_level_ratio,
                " ng (", nrow(qc_drift_ratio), " points; ",
                n_levels_qc_ratio - 1, " lower QC level(s) excluded)")
      } else {
        message("  Ratio drift correction: using ", nrow(qc_drift_ratio),
                " QC points at ", max_qc_cal_level_ratio, " ng")
      }
      message("  Reference Cal ratio (", max_qc_cal_level_ratio, " ng): ",
              round(ref_ratio, 4))

      qc_drift_ratio <- qc_drift_ratio %>%
        select(Row, petn_ratio, Ratio_CorrFactor)

      n_qc_ratio <- nrow(qc_drift_ratio)

      # --- Step 3: Fit the drift model ---
      drift_model_ratio  <- NULL
      drift_method_ratio <- "None"

      if (n_qc_ratio >= 4) {
        drift_model_ratio <- lm(Ratio_CorrFactor ~ Row + I(Row^2), data = qc_drift_ratio)
        drift_method_ratio <- "Quadratic"

        r2_ratio <- summary(drift_model_ratio)$r.squared
        if (is.na(r2_ratio) || r2_ratio < 0.5) {
          message("  Ratio drift correction: quadratic R2 = ", round(r2_ratio, 3),
                  " (poor fit); falling back to linear.")
          drift_model_ratio  <- lm(Ratio_CorrFactor ~ Row, data = qc_drift_ratio)
          drift_method_ratio <- "Linear"
        }

      } else if (n_qc_ratio == 3) {
        drift_model_ratio  <- lm(Ratio_CorrFactor ~ Row + I(Row^2), data = qc_drift_ratio)
        drift_method_ratio <- "Quadratic"
        message("  Ratio drift correction: only 3 QC points -- quadratic fit is exact (0 residual df).")

      } else if (n_qc_ratio == 2) {
        drift_model_ratio  <- lm(Ratio_CorrFactor ~ Row, data = qc_drift_ratio)
        drift_method_ratio <- "Linear"
        message("  Ratio drift correction: only 2 QC points -- using linear interpolation.")

      } else {
        message("  Ratio drift correction: fewer than 2 usable QC points -- skipping correction.")
      }

      # --- Step 4: Apply ratio correction and re-quantify ---
      if (!is.null(drift_model_ratio)) {
        predicted_cf_ratio <- predict(drift_model_ratio, newdata = data.frame(Row = Combined$Row))

        # Anchor normalisation: force CF = 1.0 at Cal set 1 position
        cal_rows <- Combined$Row[Combined$Type == "Cal" & Combined$CalibrationSet == 1]
        if (length(cal_rows) > 0) {
          anchor_row <- mean(cal_rows)
          anchor_cf  <- predict(drift_model_ratio, newdata = data.frame(Row = anchor_row))
          if (is.finite(anchor_cf) && anchor_cf > 0) {
            predicted_cf_ratio <- predicted_cf_ratio / anchor_cf
            message("  Ratio drift correction: anchored to Cal set 1 (mean Row ",
                    round(anchor_row, 1), ", raw CF = ",
                    round(anchor_cf, 4), " -> normalised to 1.0)")
          }
        }
        ...
        # Sanity: clamp correction factors
        if (any(predicted_cf_ratio < 0, na.rm = TRUE)) {
          warning("  Ratio drift correction: some predicted correction factors were negative -- clamping to 1.0.")
          predicted_cf_ratio[predicted_cf_ratio < 0] <- 1.0
        }
        if (any(predicted_cf_ratio > 3.0, na.rm = TRUE)) {
          warning("  Ratio drift correction: some correction factors exceed 3.0 -- severe drift detected.")
        }

        Combined$petn_ratio_drift_correction_factor <- predicted_cf_ratio
        Combined$petn_ratio_drift_model             <- drift_method_ratio
        Combined$petn_ratio_dc                      <- Combined$petn_ratio * predicted_cf_ratio

        # Re-quantify drift-corrected ratio using stored ratio calibration models
        for (model_idx in seq_along(stored_petn_ratio_models)) {
          m <- stored_petn_ratio_models[[model_idx]]
          ...
          for (j in m$rows) {
            ...
            if (!is.na(row_snr_flag) && row_snr_flag == "Below_LOD") {
              Combined$petn_concentration_ratio_dc[j] <- NA_real_
              Combined$petn_quant_method_ratio_dc[j]  <- "Below_LOD"
              Combined$petn_range_flag_ratio_dc[j]    <- NA_character_
              next
            }
            if (!is.na(row_snr_flag) && row_snr_flag == "Below_LOQ") {
              Combined$petn_concentration_ratio_dc[j] <- NA_real_
              Combined$petn_quant_method_ratio_dc[j]  <- "Below_LOQ"
              Combined$petn_range_flag_ratio_dc[j]    <- NA_character_
              next
            }

            res <- solve_concentration(
              Combined$petn_ratio_dc[j],
              m$lin_model,
              m$cal_y_range,
              m$quad_model
            )
            Combined$petn_concentration_ratio_dc[j] <- res$value
            Combined$petn_quant_method_ratio_dc[j]  <- res$method
            Combined$petn_range_flag_ratio_dc[j]    <- res$range_flag
          }
        }

        # --- Console summary ---
        r2_val_ratio <- round(summary(drift_model_ratio)$r.squared, 4)
        message("  PETN ratio drift correction applied:")
        message("    Model: ", drift_method_ratio)
        message("    QC points used: ", n_qc_ratio)
        message("    R-squared: ", r2_val_ratio)
        message("    Ratio correction factor range: ",
                round(min(predicted_cf_ratio, na.rm = TRUE), 4), " - ",
                round(max(predicted_cf_ratio, na.rm = TRUE), 4))

        # --- Ratio Diagnostic plot ---
        pred_line_ratio <- data.frame(
          Row = seq(min(Combined$Row), max(Combined$Row), length.out = 200)
        )
        pred_line_ratio$Ratio_CorrFactor <- predict(drift_model_ratio, newdata = pred_line_ratio)

        sample_points_ratio <- data.frame(
          Row = Combined$Row,
          Ratio_CorrFactor = predicted_cf_ratio,
          Type = Combined$Type
        )

        p_drift_ratio <- ggplot() +
          geom_line(data = pred_line_ratio,
                    aes(x = Row, y = Ratio_CorrFactor),
                    colour = "darkgreen", linewidth = 1) +
          geom_point(data = qc_drift_ratio,
                     aes(x = Row, y = Ratio_CorrFactor),
                     colour = "red", size = 4, shape = 17) +
          geom_point(data = sample_points_ratio %>% filter(Type == "Sample"),
                     aes(x = Row, y = Ratio_CorrFactor),
                     colour = "grey50", size = 2, alpha = 0.6) +
          geom_hline(yintercept = 1.0, linetype = "dashed", colour = "grey40") +
          theme_bw() +
          labs(
            title = paste0("PETN Ratio Drift Correction (", drift_method_ratio,
                           ", R\u00B2 = ", r2_val_ratio, ")"),
            subtitle = paste0("QC ratio correction factors (red triangles), ",
                              "fitted model (green line), sample positions (grey dots)"),
            x = "Injection Number",
            y = "Ratio Correction Factor"
          )

        ggsave(
          "PETN_Ratio_DriftCorrection.png",
          p_drift_ratio,
          path = Results.dir,
          width = 10, height = 5, dpi = 300
        )
        ...
        Combined$petn_ratio_drift_model <- "None"
      }

    } else {
      message("  Ratio drift correction: reference Cal ratio is invalid -- skipping correction.")
      Combined$petn_ratio_drift_model <- "None"
    }
  } else {
    message("  Ratio drift correction: no usable QC rows -- skipping correction.")
    Combined$petn_ratio_drift_model <- "None"
  }

} else if (!apply_petn_drift_correction) {
  Combined$petn_ratio_drift_model <- "Disabled"
  message("  PETN ratio drift correction: disabled (apply_petn_drift_correction = FALSE).")
}
...
# --- Drift-corrected derived columns for PETN Ratio ---
Combined$petn_sample_conc_ratio_dc     <- Combined$petn_concentration_ratio_dc / Dilution
Combined$petn_mass_in_sample_ratio_dc  <- Combined$petn_sample_conc_ratio_dc * SampleVol
Combined$petn_percent_recovery_ratio_dc <- Combined$petn_mass_in_sample_ratio_dc / DepositMass * 100

Combined$petn_percent_bias_ratio_dc <- ifelse(
  Combined$Type %in% c("Cal", "QC") &
    !is.na(Combined$petn_concentration_ratio_dc) &
    !is.na(Combined$TrueCalConcAdj),
  (Combined$petn_concentration_ratio_dc - Combined$TrueCalConcAdj) / Combined$TrueCalConcAdj * 100,
  NA_real_
)

Combined$petn_qc_flag_ratio_dc <- ifelse(
  Combined$Type == "QC" & !is.na(Combined$petn_percent_bias_ratio_dc),
  ifelse(abs(Combined$petn_percent_bias_ratio_dc) <= qc_bias_limit, "PASS", "FAIL"),
  NA_character_
)
...
# =========================================================
# QC Accuracy Plots (per analyte, 4 facets for PETN)
# =========================================================
# Generates a faceted plot showing QC percent bias across the
# run with acceptance limits (+/- qc_bias_limit) for each
# calibration method.
#
# PETN facets: Peak Area | PA/15N-RDX IS Ratio |
#              Peak Area (Drift-Corrected) | Ratio (Drift-Corrected)
# RDX facets:  Peak Area | PA/15N-RDX IS Ratio
...
if (nrow(qc_plot_data) > 0 && has_calibration) {
  ...
  for (analyte_name in names(analytes)) {
    ...
    # Drift-corrected Ratio facet (PETN only)
    if (prefix == "petn" &&
        "petn_percent_bias_ratio_dc" %in% names(qc_plot_data) &&
        !all(is.na(qc_plot_data$petn_percent_bias_ratio_dc))) {
      ratio_dc_df <- qc_plot_data %>%
        select(Row, CalLevel,
               PercentBias = petn_percent_bias_ratio_dc,
               QCFlag      = petn_qc_flag_ratio_dc) %>%
        mutate(Method = "Ratio (Drift-Corrected)")
      plot_rows[["ratio_dc"]] <- ratio_dc_df
    }
    ...
    if ("ratio_dc" %in% names(plot_rows)) {
      method_order <- c(method_order, "Ratio (Drift-Corrected)")
    }
...
if ("QC" %in% Combined$Type && has_calibration) {
  ...
  # Add drift-corrected columns for PETN
  qc_export_cols <- c(qc_export_cols,
    "petn_drift_correction_factor", "petn_drift_model",
    "petn_pa_dc", "petn_concentration_pa_dc",
    "petn_percent_bias_pa_dc", "petn_qc_flag_pa_dc",
    "petn_ratio_drift_correction_factor", "petn_ratio_drift_model",
    "petn_ratio_dc", "petn_concentration_ratio_dc",
    "petn_percent_bias_ratio_dc", "petn_qc_flag_ratio_dc"
  )
