...
for (file in filenameGcData) {
  ...
  SignalMinusBg46 <- SignalMinusBg46 %>%
    filter(
      between(RTime, (PETN.Exp.Range - 2), (PETN.Exp.Range + 2)) |
        between(RTime, (IS.Exp.Range - 2), (IS.Exp.Range + 2)))
  ...
  Step.size <- DataSIM46$RetentionTime[6] - DataSIM46$RetentionTime[5]
  ...
  # )
  # 
  p <- ggplot(Combined.bc.fillPeak46, aes(x = RetentionTime, y = Subtracted)) +
     geom_line(color = "red") +
     labs(
  ...
  # # When running single file don't run line below (skip to ggsave)
   #  GcDataName <- gsub('.{4}$', '', GcDataName)
   ggsave(
     sprintf("%s_SIM_GC_Traces.tiff",GcDataName),
     plot = p,
     device = NULL,
     path = file.path(GCSampleTrace.dir),
     scale = 1,
  ...
  if (is.na(Q.IS.testRange) || Q.IS.testRange < 10){
    ...
    PeakIS <- SignalMinusBg46 %>%
      filter( between(RTime, Peak.IS.Lmin, Peak.IS.Lmax) )  # 287, 294
