### Before Image Analysis - Analyze Before-swab threshold results ###
...
for (file in BeforeThresholdResults_files) {
  ...
  # Extract the filename without extension and remove "_Before" suffix
  results_name <- tools::file_path_sans_ext(basename(file))
  results_name <- sub("_Before$", "", results_name)
  
  # Add proper threshold values (1-100) since CSV may have empty strings
  tempresults$Threshold <- 1:100
  
  # Rename the Area column to the sample identifier
