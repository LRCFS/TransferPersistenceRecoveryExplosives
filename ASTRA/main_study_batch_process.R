...
# MAIN STUDY VARIANT - adapted from time_based_batch_process_adaptive_DISTANCE.R
# (the Pilot Study script), which is left untouched as the working
# reference for the Pilot Study. This variant processes the Main Study
# pressure traces (MAIN_001-044) instead, and generalizes several places
# that were hardcoded for the Pilot Study's 2 pressure levels (50g/200g)
# to instead support the Main Study's 5 pressure levels (10/50/100/200/300g).
# See MAIN_STUDY_ADAPTATIONS comments throughout for what changed and why.
#
...
# Author: OpenCode
# Date: 2026-08-25
# Version: v1.0.0-MAIN-STUDY (adapted from Pilot v1.3.0-CONSOLIDATED)
...
# Metadata file path
# MAIN_STUDY_ADAPTATION: points at the Main Study nested metadata/recovery
# table instead of the Pilot Study one. Must contain RunID + Pressure_g
# columns (checked in build_sample_config()); this file has 54 rows
# (44 Main + 10 NC), but only RunID matching the MAIN_[0-9]+ pattern below
# will actually be configured for processing (NC rows are skipped because
# no MAIN_ABSBatchN_NC.csv-style file matches that numeric pattern, and
# because their Pressure_g is NA anyway).
METADATA_FILE <- "C:/Users/A Bruce - User/OneDrive - University of Dundee/Documents/Experimental Results/ASTRA Swabbing/Main Study/main_study_data_nested.csv"

# Pressure-level configurations (unified defaults)
# MAIN_STUDY_ADAPTATION: the Pilot Study only used 2 pressure levels
# (50g/200g). The Main Study design uses 5 (10/50/100/200/300g -
# VeryLow/Low/Medium/High/VeryHigh). The 50g/200g entries are kept
# unchanged from the Pilot script; the 10/100/300g entries below are new.
#
# stable_low/stable_high here are only a starting point: with
# use_adaptive_thresholds = TRUE (below), the actual per-sample band is
# auto-discovered from each trace's first ~60s (see
# discover_stable_pressure_range()/calculate_adaptive_thresholds()) and
# these config values are only used as a fallback if that discovery
# fails. expected_period is likewise only a crude starting guess used to
# find the first few cycles (refined from real data after 3 cycles are
# found, see detect_initial_cycles()) - so it does not need to be exact,
# just within ~2.5s (half the 5s initial_search_window) of the true
# period. No G-code files for the 10g/100g/300g protocols were found in
# the repo to derive an exact expected cycle time from, so these are
# interpolated estimates between the known 50g (5s) and 200g (10s)
# values; verify against the console cycle-detection log for the first
# few samples at each new level and adjust here if cycles are being
# missed.
PRESSURE_CONFIGS <- list(
  "300" = list(  # 300g very-high pressure samples (Main Study only)
    target = 300,
    stable_low = 260,
    stable_high = 340,
    # RETUNED after first batch run: initial guess of 12s caused 10/12
    # 300g samples to fail with "Could not detect enough initial cycles"
    # (cycle 2 not found within the +/-2.5s search window around
    # last_start + 12). The one 300g sample that *did* succeed
    # (MAIN_012) showed a real measured inter-cycle interval of ~14.1s
    # (16.298s -> 30.412s -> 44.531s, i.e. ~14.11-14.12s), so 12s was
    # simply the wrong initial guess for this level - not a detection
    # bug. Updated to 14 (within 0.1-0.2s of the observed value, well
    # inside the +/-2.5s window), which should let Phase 1 find cycle 2
    # for the previously-failing samples too.
    expected_period = 14
  ),
  ...
  "100" = list(  # 100g medium pressure samples (Main Study only)
    target = 100,
    stable_low = 85,
    stable_high = 115,
    expected_period = 7
  ...
  "10" = list(   # 10g very-low pressure samples (Main Study only)
    target = 10,
    stable_low = 7,
    stable_high = 14,
    expected_period = 4
...
#
# MAIN_STUDY_ADAPTATION: the Pilot Study's two known low-plateau
# exceptions (PILOT_014/PILOT_018) do not apply to the Main Study dataset
# (different sample IDs entirely) and have been removed. Two Main Study
# exceptions were found after the first two batch runs both failed on
# these samples with "Could not find any stable region after startup" -
# in both cases the adaptive discovery band (target*0.85-1.10) AND the
# PRESSURE_CONFIGS default fallback band above were too narrow/mis-
# centered for this specific sample's actual achieved pressure, confirmed
# by directly inspecting the raw trace (binning load by 20s windows
# across the full recording, filtered to contact-range points):
#   MAIN_030 (10g target): achieved pressure drifts from ~16g in the
#     first ~60s down to ~12g by the end (~200s), staying roughly within
#     11-17g throughout - well above the 10g adaptive discovery band
#     (8.5-11g) and mostly above the default fallback (7-14g) too.
#   MAIN_043 (50g target): achieved pressure sits consistently around
#     ~38-40g mean in every 20s bin across the full ~300s recording -
#     below both the 50g adaptive discovery band (42.5-55g) and the
#     default fallback (45-65g). Directly analogous to PILOT_014's
#     ~36-37g low-plateau at the same nominal 50g target.
# As with the Pilot Study's overrides, `target` is intentionally left at
# the nominal/intended protocol value (10 / 50) - only the achieved
# stable band is overridden, so Mean_Pressure in the batch summary for
# these two samples will legitimately read well off nominal; treat them
# accordingly in any "at-target" aggregate statistics (both will also be
# picked up by Flag_15Pct_Outlier). No physical/mechanical cause for
# either deviation has been independently confirmed - only that it is a
# genuine, consistent characteristic of that specific recording rather
# than a period/threshold detection bug (both stayed in a fairly tight,
# non-noisy band across the *entire* recording, not just a brief blip).
# Add further MAIN_xxx overrides here in the same style if any other
# sample reports similar failures on future re-runs.
SAMPLE_OVERRIDES <- list(
  "MAIN_030" = list(stable_low = 10, stable_high = 18),
  "MAIN_043" = list(stable_low = 34, stable_high = 46)
)
...
PARAMS <- list(
  # File paths
  # MAIN_STUDY_ADAPTATION: points at the Main Study Pressure Traces folder
  # (all MAIN_001-044 + MAIN_*_NC.csv raw traces were consolidated into
  # this single folder; previously MAIN_001-024 lived under the Pilot
  # Study folder instead). Output goes to a new ProcessedData subfolder
  # here, kept separate from the Pilot Study's ProcessedData.
  data_dir = "C:/Users/A Bruce - User/OneDrive - University of Dundee/Documents/Experimental Results/ASTRA Swabbing/Main Study/Pressure Traces",
  output_dir = "C:/Users/A Bruce - User/OneDrive - University of Dundee/Documents/Experimental Results/ASTRA Swabbing/Main Study/Pressure Traces/ProcessedData",
  ...
  # MAIN_STUDY_ADAPTATION: the Pilot script picked between just two
  # named minimum-tolerance constants (adaptive_tolerance_200g /
  # adaptive_tolerance_50g) via a single `target >= 150` check in
  # calculate_adaptive_thresholds(). That binary split can't represent 5
  # levels, so it's replaced by a per-target lookup table
  # (adaptive_tolerance_by_target) used directly by target string below;
  # the 50g/200g values (8/10) are unchanged from the Pilot script, and
  # 10/100/300g are new linear-ish interpolations/extrapolations pending
  # empirical review once real Main Study traces have been processed.
  adaptive_tolerance_by_target = list(
    "10"  = 3,
    "50"  = 8,
    "100" = 9,
    "200" = 10,
  ...
  transient_comparison_dir = file.path(
    "C:/Users/A Bruce - User/OneDrive - University of Dundee/Documents/Experimental Results/ASTRA Swabbing/Main Study/Pressure Traces/ProcessedData",
    "TransientComparison"
  )
)
...
build_sample_config <- function(metadata_file, data_dir, pressure_configs, overrides) {
  ...
  # MAIN_STUDY_ADAPTATION: matches MAIN_001.csv ... MAIN_044.csv only.
  # This pattern intentionally excludes the negative-control traces
  # (MAIN_ABSBatch1_NC.csv, MAIN_SteelBatch3_NC.csv, etc.), since those
  # have no Pressure_g/recovery values in the metadata to plot against.
  csv_files <- list.files(data_dir, pattern = "^MAIN_[0-9]+\\.csv$", full.names = FALSE)
...
calculate_adaptive_thresholds <- function(pressure_stats, config, params) {
  ...
  # MAIN_STUDY_ADAPTATION: minimum is now looked up per-target from
  # PARAMS$adaptive_tolerance_by_target (5 levels) instead of the Pilot
  # script's binary `target >= 150` check between two named constants
  # (adaptive_tolerance_200g/50g), preventing too-narrow ranges for
  # whichever protocol this sample belongs to.
  # Maximum: 20g (prevent too wide ranges that capture transients)
  target_key <- as.character(config$target)
  min_tolerance <- if (target_key %in% names(params$adaptive_tolerance_by_target)) {
    params$adaptive_tolerance_by_target[[target_key]]
  } else {
    cat(sprintf("  WARNING: No adaptive_tolerance_by_target entry for target %sg, defaulting to 10g\n", target_key))
    10
  }
  max_tolerance <- 20
...
predict_and_detect_remaining <- function(data, initial_cycles, period_stats, params) {
  ...
  # BUG FIX: if Phase 1 only found exactly 2 initial cycles (1 interval),
  # sd() of a single value is NA, which propagates through
  # `period_tolerance_factor * sd_period` and max(NA, ...) to make
  # `tolerance` itself NA - every subsequent search_start/search_end
  # becomes NA, detect_stable_by_threshold() then can't find anything,
  # every remaining cycle gets FORCED with duration=0, and
  # calculate_statistics() ends up with zero real_cycles rows ("no rows
  # to aggregate", observed for Main Study sample MAIN_024). This can't
  # happen with >= 3 initial cycles (>= 2 intervals, sd() well-defined),
  # which is the normal case; guard the degenerate 1-interval case by
  # falling back directly to min_period_tolerance instead of computing
  # a not-a-number tolerance from it.
  sd_period_for_tolerance <- if (is.na(period_stats$sd_period)) 0 else period_stats$sd_period
  tolerance <- max(
    params$period_tolerance_factor * sd_period_for_tolerance,
  ...
  # ROBUSTNESS FIX (discovered while running this script against the Main
  # Study 10g-target samples, e.g. MAIN_008): if the Phase 2 period
  # estimate degenerates to a small/wrong value (e.g. because the initial
  # 3-5 cycles overlapped instead of being cleanly separated - seen for a
  # 10g sample where the adaptive stable band was wide enough that
  # "cycles" 1-5 detected overlapping ~5s regions instead of distinct
  # ~1s passes, producing a negative interval and a ~0.45s period
  # estimate), `last_start` can land on a *fixed point*: the same real
  # stable region keeps getting re-found inside every subsequent search
  # window regardless of where that window is centered, so `last_start`
  # never advances and predicted_start never grows enough to exceed
  # max_time - the original `while (TRUE)` loop would then run
  # essentially forever (observed: >60,000 identical iterations after
  # 20 minutes with no sign of terminating). Two independent guards are
  # added below: a hard iteration cap (generously above the ~40-90
  # cycles any real sample should need) and an explicit stall detector
  # that aborts as soon as last_start fails to advance for a few
  # consecutive iterations, since that specific failure mode will never
  # self-correct. Either guard tripping returns whatever cycles were
  # collected so far with a warning, rather than hanging; the sample
  # will likely show as a low-quality/failed detection in
  # batch_summary.csv, same as any other detection failure, and can be
  # investigated/fixed via SAMPLE_OVERRIDES same as the Pilot Study's
  # PILOT_014/018 cases.
  ...
  while (TRUE) {
    ...
    if (cycle_count > max_total_cycles) {
      warning(sprintf(
        "predict_and_detect_remaining: exceeded max_total_cycles (%d) without reaching end of data (%.1fs) - aborting to avoid an unbounded loop. Likely cause: degenerate period estimate (period=%.3fs). Returning cycles collected so far.",
        max_total_cycles, max_time, period_stats$period
      ))
      break
    }
    ...
    pre_iteration_last_start <- last_start
    ...
    # Stall detection (see ROBUSTNESS FIX comment above the while loop):
    # if last_start failed to advance at all this iteration, the same
    # region will very likely be re-found again next iteration too since
    # predicted_start (and therefore the search window) barely moved -
    # this is the degenerate fixed-point case, not just normal noise, so
    # a small number of consecutive non-advances is treated as fatal
    # rather than retried indefinitely.
    if (last_start <= pre_iteration_last_start) {
    ...
    if (stall_count >= max_stall_iterations) {
      warning(sprintf(
        "predict_and_detect_remaining: last_start failed to advance for %d consecutive iterations (stuck at %.3fs, period=%.3fs) - aborting to avoid an unbounded loop. Returning cycles collected so far.",
        stall_count, last_start, period_stats$period
      ))
      break
    }
  }
...
process_sample <- function(sample_name, config, params) {
  ...
  if (!file.exists(data_file)) {
    ...
    # BUG FIX: see the "Could not detect enough initial cycles" comment
    # further down for why a raw NULL return here would silently delete
    # this sample from main_batch()'s `results` list entirely. Returning
    # a proper failure list instead keeps it visible in batch_summary.csv.
    return(list(sample = sample_name, target = config$target, success = FALSE, error = "File not found"))
  }
  ...
  if (is.null(data)) {
    ...
    return(list(sample = sample_name, target = config$target, success = FALSE, error = "Could not read/clean CSV file"))
  }
  ...
  tryCatch({
    ...
    if (nrow(initial_cycles) < 2) {
      # MAIN_STUDY_ADAPTATION / BUG FIX: this used to `return(NULL)`
      # directly. That NULL then got assigned via
      # `results[[sample_name]] <- NULL` in main_batch()'s processing
      # loop, which in R *deletes* that key from the list rather than
      # storing a NULL value in it - silently dropping the sample from
      # `results` entirely (no row in batch_summary.csv, not even a
      # Success=FALSE one, and excluded from any plotting/target-listing
      # code that iterates over `results`). This was never observed with
      # the Pilot Study's 27 samples, but 8 of the Main Study's 44
      # samples hit exactly this "not enough initial cycles" condition
      # (e.g. MAIN_002, MAIN_004, MAIN_006 among others) and were
      # silently going missing. Using stop() instead routes this through
      # the same tryCatch(..., error = function(e) {...}) handler below
      # that the "Could not find any stable region" failure already
      # uses, which returns a proper list(success = FALSE, target = ...,
      # error = ...) - consistent with every other failure mode, and
      # guaranteed to still appear in the batch summary.
      stop(sprintf("Could not detect enough initial cycles for %s (found %d, need >= 2)", sample_name, nrow(initial_cycles)))
    }
...
plot_stable_regions_overlay <- function(sample_id, sample_result, data_dir, output_dir, params, gcode_params) {
  ...
  # MAIN_STUDY_ADAPTATION: the Pilot script's binary `target >= 150`
  # check only distinguished 50g vs 200g. Replaced with an explicit
  # per-target lookup covering all 5 Main Study levels; margins follow
  # the same style as the original two entries (roughly proportional,
  # widening in absolute terms at higher targets) rather than a fixed
  # formula. Falls back to a generic +/-40% window for any unexpected
  # target not in the table.
  y_limits_by_target <- list(
    "10"  = c(0, 20),
    "50"  = c(30, 70),
    "100" = c(60, 140),
    "200" = c(150, 250),
  ...
  target_key <- as.character(target_pressure)
  if (target_key %in% names(y_limits_by_target)) {
    y_limits <- y_limits_by_target[[target_key]]
  } else {
    y_limits <- c(target_pressure * 0.6, target_pressure * 1.4)
  }
...
# Mean_Pressure against its own Target, faceted by Target level (each of
# the 5 Main Study levels evaluated separately since their absolute
# pressures aren't comparable; facet_wrap(..., scales = "free") below
# already generalizes to any number of distinct Target values, so no
# Main-Study-specific change was needed in this function itself).
...
# the table; this makes outliers visible at a glance (see main_batch()'s
# Flag_15Pct_Outlier comment for the Pilot Study's known examples of this -
# equivalent Main Study outliers, if any, will only be known once this
...
create_all_comparison_plots <- function(results, metadata_df, data_dir, output_dir, params, gcode_params,
  ...
  # MAIN_STUDY_ADAPTATION: the Pilot script hardcoded exactly two target
  # values (50, 200) here since that's all the Pilot Study used. The
  # Main Study has 5 (10/50/100/200/300), so this now derives the actual
  # set of targets present from the processed results instead, sorted
  # ascending for a predictable plot generation order.
  present_targets <- sort(unique(sapply(results, function(r) r$target)))
  
  for (target_pressure in present_targets) {
    file_target <- plot_full_traces_comparison(target_pressure, results, metadata_df, data_dir, params, gcode_params)
    if (!is.null(file_target)) {
      generated_files <- c(generated_files, file_target)
    }
  }
...
main_batch <- function() {
  ...
  for (sample_name in names(SAMPLE_CONFIG)) {
    ...
    # DEFENSIVE BUG FIX: `results[[sample_name]] <- NULL` in R deletes
    # that key instead of storing NULL, which would silently drop the
    # sample from every downstream step (summary table, target-listing
    # for plots, etc.) with no warning. process_sample() has been fixed
    # to never return a bare NULL (see its own comments), but this is a
    # cheap extra guard against any future code path doing so.
    if (is.null(result)) {
      warning(sprintf("process_sample('%s') returned NULL directly (should return a failure list instead) - substituting a placeholder failure result so this sample is not silently dropped.", sample_name))
      result <- list(sample = sample_name, target = config$target, success = FALSE, error = "process_sample returned NULL")
    }
  ...
  # For the Pilot Study, at this same default 15% threshold this flagged
  # PILOT_014 (-30.0%), PILOT_018 (-18.6%), PILOT_011 (+15.7%), and
  # PILOT_030 (-15.2%) - see the Pilot script's version of this comment.
  # No physical/mechanical cause was independently confirmed for any of
  # those four; it was a measured statistical deviation only, not a
  # diagnosis. Whether/which Main Study samples get flagged here is not
  # yet known - check the printed summary_data / batch_summary.csv after
  # running this script, and consider adding a SAMPLE_OVERRIDES entry
  # (see top of file) for any that turn out to be a genuine achieved-
  # pressure plateau on inspection of the raw trace, same as was done
  # for the Pilot Study's two low-plateau samples. Failed samples get NA.
  ...
  # MAIN_STUDY_ADAPTATION: the Pilot script hardcoded 8 explicit blocks
  # covering exactly {50g, 200g} x {Overall, Wet, Dry}, since that's all
  # 2 levels x conditions the Pilot Study had. The Main Study has 5
  # pressure levels, and (per main_study_data_nested.csv) all 44 Main
  # samples are Solvent_level == "present" (Solvent_detail ==
  # "100uL_EtOH") - there is no "dry" condition in the Main Study design.
  # Rather than hardcode a second, different set of blocks, this loops
  # dynamically over whatever Target values and Solvent_detail values are
  # actually present in the data, computing "Overall" (all successful
  # samples at that target) plus one row per distinct Solvent_detail
  # value found (only "Wet"/100uL_EtOH in practice for the Main Study,
  # but this will also correctly handle a "dry" condition or additional
  # solvent types if the design changes later).
  compute_overall_stats_row <- function(data_subset, pressure_level_label, condition_label) {
    if (nrow(data_subset) == 0) return(NULL)
    data.frame(
      Pressure_Level = pressure_level_label,
      Condition = condition_label,
      Mean_Pressure = mean(data_subset$Mean_Pressure, na.rm = TRUE),
      SD_Pressure = sd(data_subset$Mean_Pressure, na.rm = TRUE),
      RSD_Pressure = (sd(data_subset$Mean_Pressure, na.rm = TRUE) / mean(data_subset$Mean_Pressure, na.rm = TRUE)) * 100,
      Min_Pressure = min(data_subset$Mean_Pressure, na.rm = TRUE),
      Max_Pressure = max(data_subset$Mean_Pressure, na.rm = TRUE),
      N_Samples = nrow(data_subset),
      stringsAsFactors = FALSE
    )
  }
  
  present_targets_for_stats <- sort(unique(summary_with_metadata$Target[summary_with_metadata$Success == TRUE]))
  ...
  for (target_level in present_targets_for_stats) {
    pressure_level_label <- sprintf("%sg", target_level)
    data_this_target <- summary_with_metadata %>% filter(Target == target_level & Success == TRUE)
    
    # Overall (all conditions combined) for this target
    overall_stats <- rbind(overall_stats, compute_overall_stats_row(data_this_target, pressure_level_label, "Overall"))
    
    # One row per distinct Solvent_detail value observed at this target
    # (labelled "Wet" for the known 100uL_EtOH value and "Dry" for the
    # known "dry" value, matching the Pilot script's labels; any other
    # value found is used verbatim as the label so nothing is silently
    # dropped if the solvent design changes).
    solvent_values <- unique(na.omit(data_this_target$Solvent_detail))
    for (solvent_value in solvent_values) {
      condition_label <- if (solvent_value == "100uL_EtOH") {
        "Wet"
      } else if (solvent_value == "dry") {
        "Dry"
      } else {
        solvent_value
      }
      data_this_condition <- data_this_target %>% filter(Solvent_detail == solvent_value)
      overall_stats <- rbind(overall_stats, compute_overall_stats_row(data_this_condition, pressure_level_label, condition_label))
    }
  }
